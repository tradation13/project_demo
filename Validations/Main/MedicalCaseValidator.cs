...
namespace IPTS.Validators
{
    public class MedicalCaseValidator : AbstractValidator<MedicalCaseViewModel>
    {
        public MedicalCaseValidator(LocService localizer)
        {
            ...
            RuleFor(x => x.MedicalConditionId).GreaterThan(0).WithMessage(x => localizer.GetSystem("MedicalConditionRequired"));
            RuleFor(x => x.Name).MaximumLength(200).WithMessage(x => localizer.GetSystem("NameMaxLength"));
