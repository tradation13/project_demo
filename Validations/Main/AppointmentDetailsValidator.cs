...
{
    public class AppointmentDetailsValidator : AbstractValidator<AppointmentDetailsViewModel>
    {
        public AppointmentDetailsValidator(LocService localizer)
        {
            ...
            RuleFor(x => x.DoctorName).NotEmpty().WithMessage(x => localizer.GetSystem("DoctorNameRequired")).MaximumLength(150).WithMessage(x => localizer.GetSystem("DoctorNameMaxLength"));
